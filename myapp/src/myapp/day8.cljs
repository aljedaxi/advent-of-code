...
(defn splitAndTakeSecond
  [s]
  (let [xs (split s #" \| ")]
    (xs 1)))

(defn findEasyNumbers
  [s]
  (->> s
    words
    (filter #(easyDigits (count %)))
    count))
...
(defn processTestData 
  ...
  (->> s
    lines
    (map trim)
    (map splitAndTakeSecond)))

(def algo1 #(->> % processTestData (map findEasyNumbers) sum))

(def algo2 #(->> % processTestData (map findAllNumbers) sum))

(def result (algo2 testData))
